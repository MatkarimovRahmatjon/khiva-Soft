<template>
    <div>
        <div class="container my-32">
            <div class="flex  justify-between mb-24">
                <div class="my-14 max-w-[623px] w-full">
                    <h3 class=" font-medium text-[18px] leading-8">Use Client-first</h3>
                    <h1 class="my-4 font-semibold text-[38px] leading-[56px]">Top agencies and freelancers around the world use 
                        Client-first </h1>
                    <p class="opacity-[70%] text-[16px] leading-7">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation.</p>
                </div>
                <div>
                    <img src="../../../public/image/1/1.png" alt="">
                </div>
            </div>

        </div>
    </div>
</template>

<script setup>
</script>

<style lang="scss" scoped>

</style>